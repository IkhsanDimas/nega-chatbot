    ...
    <ScrollArea className="flex-1 p-4 h-full bg-[#020617]">
      <div className="flex flex-col space-y-1 pb-4">
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex w-full group ${isUser ? 'justify-end' : 'justify-start'} mb-1`}>
              <div className={`flex max-w-[85%] md:max-w-[75%] gap-1.5 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                
                {/* AVATAR - Hanya tampilkan untuk AI */}
                {!isUser && (
                  <Avatar className="w-6 h-6 mt-0 border border-white/10 shadow-sm shrink-0">
                ...
                <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} min-w-0 flex-1 ${isUser ? 'pr-0' : 'pl-0'}`}>
                    ...
                    <div className="relative group/bubble max-w-full">
                      ...
                      {isUser && !msg.file_url && (
                        <button
                          ...
                          className="absolute -left-6 top-1 p-1 text-slate-500 hover:text-cyan-400 opacity-0 group-hover/bubble:opacity-100 transition-all transform hover:scale-110 bg-[#020617]/80 rounded-full"
                          title="Edit pesan"
                        >
                  ...
                  <span className="text-[9px] text-slate-500 mt-0 px-1 select-none">
        ...
        {isLoading && (
          <div className="flex justify-start w-full animate-pulse pl-7 mb-1">
            <div className="bg-[#1e293b] px-3 py-2 rounded-2xl rounded-tl-none border border-white/5 flex items-center gap-2">
              <Loader2 className="w-3 h-3 animate-spin text-blue-500" />
              <span className="text-[11px] text-slate-400">{AI_MODEL_NAME} sedang berpikir...</span>
            </div>
          </div>
        )}
